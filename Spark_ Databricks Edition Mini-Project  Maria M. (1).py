...
result = spark.sql("""
   SELECT education, MAX(bachelor_rate) AS bachelor_rate
FROM (
    SELECT
        education,
        ROUND(SUM(IF(LTRIM(marital_status) = 'Never-married', 1, 0)) / COUNT(*), 2) AS bachelor_rate
    FROM adult
    GROUP BY education
) subquery
GROUP BY education;
...
# do grouping (and a round)
df_result = df_result.groupBy('education').agg(round(mean('is_divorced'),2).alias('divorced_rate'))
# do ordering
df_result = df_result.orderBy(desc('divorced_rate'))
# show results
df_result.show(1)
...
### Question 7.2 Answer ###
bestMaxDepth = gbCvModel.bestModel.getOrDefault("maxDepth")
bestMaxIter = gbCvModel.bestModel.getOrDefault("maxIter")
...
feature_coeff = pd.DataFrame([(name, lrModel.coefficients[int(idx)]) for idx, name in attrs], columns=['Feature Name', 'Feature Coefficient'])

# Sort by feature coefficient in descending order
feature_coeff = feature_coeff.sort_values(by=['Feature Coefficient'], ascending=False)
...
# Assuming 'attrs' is a list of feature names and 'gbCvModel.bestModel.featureImportances' is an array
# Use enumerate to get both the index (as an integer) and name
gbCvFeatureImportance = pd.DataFrame([(name, gbCvModel.bestModel.featureImportances[int(idx)]) for idx, name in enumerate(attrs)], columns=['feature_name', 'feature_importance'])
...
rfPredictions.limit(10).toPandas()
...
attrs = sorted(
    (attr["idx"], attr["name"]) for attr in (chain(*rfPredictions
        .schema[rfModel.getFeaturesCol()]
        .metadata["ml_attr"]["attrs"].values())))

# Create a DataFrame with feature names and feature importances
rfFeatureImportance = pd.DataFrame([(name, feature_importances[int(idx)]) for idx, name in attrs], columns=['feature_name', 'feature_importance'])

# Sort by feature importance in descending order
rfFeatureImportance = rfFeatureImportance.sort_values(by=['feature_importance'], ascending=False)
